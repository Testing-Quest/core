...
import { DirectCoherency } from '../../../../../../src/infrastructure/react/tabs/analysis/components/Plots/DirectCoherency'
...
describe('DirectCoherency Component', () => {
  const mockData = [
    { x: 1, y: 0.8, hover: 1 },
    { x: 2, y: 0.9, hover: 2 },
    { x: 3, y: 0.7, hover: 3 },
  ...
  const mockClient = {
    getDirectCoherencyData: jest.fn().mockResolvedValue({ data: mockData }),
  }
  ...
  it('shows loading state initially', () => {
    render(<DirectCoherency client={mockClient as unknown as Client} />)
  ...
  it('fetches and displays data correctly', async () => {
    ...
    expect(mockClient.getDirectCoherencyData).toHaveBeenCalled()
  ...
  it('displays correct axis labels', async () => {
    ...
    expect(axisLabels[1]).toHaveTextContent('Coherency')
  })
  ...
  it('handles error in data fetching', async () => {
    const errorClient = {
      getDirectCoherencyData: jest.fn().mockRejectedValue(new Error('Fetch error')),
    }

    const consoleSpy = jest.spyOn(console, 'error').mockImplementation()
    render(<DirectCoherency client={errorClient as unknown as Client} />)
