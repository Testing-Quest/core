...
export const DirectCoherency: React.FC<PanelProps> = ({ client }) => {
  ...
  const fetchHealth = useCallback(async () => {
    try {
      const response = await client.getDirectCoherencyData()
        ...
        <ResponsiveContainer width='90%' height={500}>
          <ScatterChart margin={{ bottom: 15 }}>
            ...
            <YAxis type='number' dataKey='y' name='Y'>
              <Label value='Coherency' angle={-90} position='insideLeft' offset={8} fontSize={fontSize} fill='' />
            </YAxis>
  ...
  if (active && payload?.length) {
    ...
    return (
      <div style={{ backgroundColor: 'white', padding: '5px', border: '1px solid #ccc' }}>
        ...
        <p>{`Coherency: ${point.y.toFixed(2)}`}</p>
