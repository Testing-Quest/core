...
export const DirectWeight: React.FC<PanelProps> = ({ client }) => {
  ...
  const fetchHealth = useCallback(async () => {
    try {
      const response = await client.getdirectWeightData()
        ...
        <ResponsiveContainer width='90%' height={500}>
          <ScatterChart margin={{ bottom: 15 }}>
            ...
            <YAxis type='number' dataKey='y' name='Y'>
              <Label value='Weighted Score' angle={-90} position='insideLeft' offset={8} fontSize={fontSize} fill='' />
            </YAxis>
