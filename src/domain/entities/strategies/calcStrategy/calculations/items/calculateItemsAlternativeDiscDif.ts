...
export function calculateItemsAlternativeDiscriminationDifficulty(
  ...
  const processAlternative = (alternative: string) => {
    const correctedMatrix = matrix.map(row =>
      row.map(item => +(item === alternative)),
    )
    const itemsDirectScore = Array.from({ length: numUsers }, (_, colIndex) =>
      correctedMatrix.reduce((acc, row) => acc + row[colIndex], 0),
    )

    alternativeDiscrimination.set(
      `Discrimination ${alternative}`,
      calculateDisrimination(correctedMatrix),
    )
    alternativeDifficulty.set(
      `Difficulty ${alternative}`,
      calculateDifficulty(itemsDirectScore),
    )
  }
