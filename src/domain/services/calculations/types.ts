export type MultipleChoiceCalculationsType = {
  ...
  itemsAltDiscDiff: (usersDirectScore: number[], matrix: string[][], numAlternatives: number) => [Map<string, number[]>, Map<string, number[]>]
  itemsConflict: (alternativeDiscrimination: Map<string, number[]>, itemDiscrimination: number[]) => boolean[]
  itemsChoice: (keys: string[], alternatives: number, alternativeDifficulty: Map<string, number[]>, numUsers: number, numItems: number) => boolean[]
